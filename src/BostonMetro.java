...
public class BostonMetro {
    ...
    /**
     * Prints the shortest path that was found between two nodes.
     * Handles printing the edges that are between same stations but with different labels.
     * Prints the stations in a linked format, not just in the arbitrary order the undirected edges hold.
     *
     * @param path
     */
        ...
        for (IEdge e : path) {
            if (prev != null && e.getNode1().equals(prev.getNode1()) && e.getNode2().equals(prev.getNode2())) {
                ...
                if (prev.getNode2().getLabel().equals(e.getNode1().getLabel())) {
                    ...
                    System.out.print("\n" + "Go from " + e.getNode2().getLabel() + " to " + e.getNode1().getLabel() + " along the "
                            + e.getLabel() + " line.");
                }
            } else {
                System.out.print("\n" + "Go from " + e.getNode1().getLabel() + " to " + e.getNode2().getLabel() + " along the "
    ...
    private void mapLines() {
		...
		/*System.out.print("this is red: " + red.values() + " \n" + "this is green: " + green.values() + " \n"
				+ "this is blue: " + blue.values() + " \n" + "this is orange: " + orange.values() + " \n");
		int total = blue.size() + green.size() + red.size() + orange.size();
