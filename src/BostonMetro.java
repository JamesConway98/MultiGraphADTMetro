...
public class BostonMetro
{
	...
	for (int i = 0; i < nodes.size(); i++)
	{
		...
		// green!!
		    ...
		    if ((nodes.get(i).contains("Green") || nodes.get(i).contains("GreenB")
			...
			if ((nodes.get(i).contains("Red") || nodes.get(i).contains("RedA")
				|| nodes.get(i).contains("RedB")) && Integer.parseInt(nodes.get(i).get(0)) == i + 1)
			{
			    ...
			    if (nodes.get(i).contains("Mattapan"))
			    {
	 ...
	 * " \n"); int total = blue.size() + green.size() + red.size() +
	 * orange.size(); System.out.print("the total is: \n" + total +"\n");
	 */
    }
